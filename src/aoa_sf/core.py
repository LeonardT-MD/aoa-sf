...
def compute_aosf(
    ...
    *,
    ...
    Rescale dict supported:
      {"mode":"none"} (default)
      {"mode":"absolute","distance": float}  # new distance from pivot for ALL 4 points

    Notes:
    - Proportional/relative rescaling is intentionally not supported.
    """
    pivot = as_xyz(pivot)
    ...
    if mode == "absolute":
        ...
        raise ValueError(f"Unknown rescale mode: {mode}. Allowed: 'none', 'absolute'.")
    ...
    # Build output without report first, then format report using the final object
    tmp = AoSFOutput(
        pivot=pivot,
    ...
    return AoSFOutput(
        **{**tmp.__dict__, "text_report": report}
    )
